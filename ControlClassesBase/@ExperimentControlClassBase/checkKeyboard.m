...
if keyIsDown
    ...
    if any(keys == key.escape)
        if isempty(timeLastEscape) || GetSecs - timeLastEscape > 0.5
            ...
            flagResult   = -3;
            xp.eyetracker.write('Pressed escape key - skip trial @%.3fsec',keyOnset)
    ...
    elseif any(keys ==  key.q)
        flagResult   = -9;
        xp.eyetracker.write('Pressed q key - quit experiment @%.3fsec',keyOnset)
        xp.eeg.eventSaveMultiIntoOne('kqui', keyOnset)

    % calibrate
    elseif any(keys ==  key.c)
        flagResult   = -1;        
        xp.eyetracker.write('Pressed c key - recalibrate @%.3fsec',keyOnset)
        xp.eeg.eventSaveMultiIntoOne('kcal', keyOnset)

    % gogogo to trial
    elseif any(keys ==  key.space) && attget
        flagResult   = 1;         
        xp.eyetracker.write('Pressed space key - go to trial @%.3fsec',keyOnset)
    ...
    elseif ~any(keys ==  KbName('LeftShift')) && any(keys ==  KbName({'1!' '2@' '3#' '4$' '5%'})', 'all')
        ...
        xp.eyetracker.write('Pressed %d key - play pause video @%.3fsec',idBreakvideo, idBreakvideo, keyOnset)
