...
%         end
        ...
        PsychPortAudio('Stop', el.calib_pahandle);  
        eyelinkanimationtarget.movie = 0;
    end


    function loadanimationmovie(el)
        % [movie movieduration fps imgw imgh] = Screen('OpenMovie',  el.window, el.calAnimationTargetFilename, 4, -1, el.calAnimationOpenSpecialFlags1);
        ...
        % [movie movieduration fps imgw imgh] = Screen('OpenMovie',  el.window, el.calAnimationTargetFilename, 4,-1,1+2,6,4);
        PsychPortAudio('FillBuffer', el.calib_pahandle, el.calib_sound);
        % PsychPortAudio('Start', el.calib_pahandle);
